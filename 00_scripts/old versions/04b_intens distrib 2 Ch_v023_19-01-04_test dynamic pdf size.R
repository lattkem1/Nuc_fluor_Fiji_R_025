...
pdf(file = file.name, width = length(glist)*3, height = length(files)%/%length(glist)*3)
...
pdf(file = file.name, width = length(glist) *3, height = 3)

par(mfrow=c(1, length(glist)),lwd = 0.3, cex = 0.5) 
