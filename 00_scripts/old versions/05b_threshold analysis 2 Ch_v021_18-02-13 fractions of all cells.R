...
scr_dir = "./00_scripts"
out_dir = "./05b_treshold analysis_2Ch"
...
load(file = paste(in_dir,"/dataset NeuN HuCD.RData",sep=""))
...
#################  main   #######################
  ...
  count.stat = mutate(Ch1.counts.r, 
                      ...
                      mean_frac_nn = NA, sd_frac_nn = NA,
                      mean_frac_np = NA, sd_frac_np = NA,
                      mean_frac_pn = NA, sd_frac_pn = NA,
                      mean_frac_pp = NA, sd_frac_pp = NA
                      )
  ...
  for (j in count.stat$group){
    ...
    t1$mean_frac_nn[1] = mean(t1$frac_nn)
    t1$sd_frac_nn[1] = sd(t1$frac_nn)
    t1$mean_frac_np[1] = mean(t1$frac_np)
    t1$sd_frac_np[1] = sd(t1$frac_np)
    t1$mean_frac_pn[1] = mean(t1$frac_pn)
    t1$sd_frac_pn[1] = sd(t1$frac_pn)
    t1$mean_frac_pp[1] = mean(t1$frac_pp)
    t1$sd_frac_pp[1] = sd(t1$frac_pp)
...
for (i in 1:length(glist)){
    ...
    nn = as_tibble(cbind(label = paste(glist[i],"_", Ch[1],"-_",Ch[2],"-",sep=""), repl = t3$frac_nn,
                         mean = t3$mean_frac_nn, sd = t3$sd_frac_nn))
    np = as_tibble(cbind(label = paste(glist[i],"_", Ch[1],"-_",Ch[2],"+",sep=""), repl = t3$frac_np,
                   mean = t3$mean_frac_np, sd = t3$sd_frac_np))
    pn = as_tibble(cbind(label = paste(glist[i],"_", Ch[1],"+_",Ch[2],"-",sep=""), repl = t3$frac_pn,
                   mean = t3$mean_frac_pn, sd = t3$sd_frac_pn))
    pp = as_tibble(cbind(label = paste(glist[i],"_", Ch[1],"+_",Ch[2],"+",sep=""), repl = t3$frac_pp,
                   mean = t3$mean_frac_pp, sd = t3$sd_frac_pp))
    t3_1 = as_tibble(rbind(nn,np,pn,pp))
...
file.name = paste(out_dir, "/fract of all cells_",Ch[1],"_",Ch[2],
                  "_thresh ", Ch.thresh[1],"_", Ch.thresh[2],".pdf", sep = "")

#plot environment: width groups number, height image number/group
pdf(file = file.name, width = 12, height = 8)
  
  pl.title = paste(Ch[1],"/",Ch[2],
                  "pos/neg _threshold ", Ch.thresh[1],"/", Ch.thresh[2], sep = "")
  y.title = paste("% of total cells")
