...
load(file = "03_R input/R rawdata set.RData")
...
load(file = paste0(in_dir,"/dataset ",Ch[1]," ", Ch[2],".RData"))
...
merge_files_to_repl = function(tab1){
  ...
  for (i in 1:length(rlist)){
    ...
    sum.sub.tab[1 ,-c(1:3)] = as.list(apply(sub.tab[,-c(1:3)],2,sum))
    sum.sub.tab
...
### calculate fraction of cells for pos/neg for both channels combined, add columns for group statistics 
  count.stat = mutate(Ch1.counts.r, 
                      ...
                      mean_frac_1n = NA_real_, sd_frac_1n = NA_real_,
                      mean_frac_1p = NA_real_, sd_frac_1p = NA_real_,
                      mean_frac_2p_of_1n = NA_real_, sd_frac_2p_of_1n = NA_real_,
                      mean_frac_2p_of_1p = NA_real_, sd_frac_2p_of_1p = NA_real_
                      )
