...
l1 = stats_list_by_repl

#stats for cell numbers

t2 = l1[[1]]
t3 = pairwise.t.test(t2$N_cells, t2$group, p.adjust.method = "none")
t4 = t3$p.value
t_stats = as_tibble(cbind(comp = "N_Cells", comp1 = colnames(t4)[1], comp2 = rownames(t4), p = t4[,1]))

#stats fract pos cells

for (i in names(l1)){
  
  t2 = l1[[i]]
  t3 = pairwise.t.test(t2$fract_pos, t2$group, p.adjust.method = "none")
  t4 = t3$p.value
  t5 = as_tibble(cbind(comp = i, comp1 = colnames(t4)[1], comp2 = rownames(t4), p = t4[,1]))
  
  t_stats = rbind(t_stats, t5)
  
}

write_csv(t_stats, file = paste0(out_dir, "/stats_ttest_",Ch,".csv"))
