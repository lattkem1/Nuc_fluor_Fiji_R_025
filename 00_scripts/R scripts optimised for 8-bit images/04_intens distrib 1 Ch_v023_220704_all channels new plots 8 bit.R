...
pl_file = lapply(Ch, function(Ch1){
  p1 = ggplot(t1, aes(x = as_vector(t1[,Ch1]), fill = group))+geom_histogram(binwidth = 2)+
    scale_x_continuous(limits = c(-1, 257), breaks = seq(0, 250, by = 50), 
                       minor_breaks = seq(0, 260, by = 5))+theme_bw()+
